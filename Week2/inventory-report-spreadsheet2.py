...
for prod in range(2, len(prod_titles) + 2):
    ...
    ws.cell(row=prod, column=2).value = prod_ids[prod - 2]
    ws.cell(row=prod, column=3).value = max_amt[prod - 2]
    ws.cell(row=prod, column=4).value = reorder_thresh[prod - 2]
    ws.cell(row=prod, column=5).value = quantity_num[prod - 2]

# save your file
wb.save("Week2\spreadsheets\inventory-spreadsheet2.xlsx")
